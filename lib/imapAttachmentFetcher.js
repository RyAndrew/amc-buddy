...
function searchUnseenMessages(config){
    return new Promise(async (resolve, reject) => {
        //,['SINCE','March 24, 2024']
...
function fetchUnseenMessages(config){
    return new Promise(async (resolve, reject) => {
        imap.openBox(process.env.IMAPDIR, async function(err, box){
            ...
            await searchUnseenMessages(config)
            resolve()
        })
    })
}
...
async function saveEmailToDatabase(email, config){
    ...
    const existingEvent = await config.db.get(`SELECT messageid FROM event where messageid=$messageid`,{$messageid:email.messageId})
...
module.exports={
    ...
    saveUnseenEventsToDbDownloadAttachments:async function(config){
        return doImapThing(fetchUnseenMessages, config)
    },
}