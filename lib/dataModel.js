...
class dataModel {
    ...
    queryEventsAndAttachmentsOffset(favoriteOnly, offset, dateStart, dateEnd){
        const pageSize = 50
        ...
        let favoriteOnlyFilter = ''
        if(favoriteOnly==='1'){
            favoriteOnlyFilter = ' and event.favorite=1 '
        }
        ...
        WHERE
            event.datetime between $dateStart and $dateEnd ${favoriteOnlyFilter}
        ORDER BY
            event.datetime DESC, eventattachment.filename ASC
        LIMIT $pageSize OFFSET $offset`
        
        return this.db.all(query,{$pageSize:pageSize, $offset:offset, $dateStart:dateStart, $dateEnd:dateEnd}
        )
        //event.eventid ASC
    }
